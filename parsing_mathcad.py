...
test_ws = mathcad_app.open(Path.cwd() / "pupa.xmcdz")
test_ws.activate()
print(f"Worksheet input names: {test_ws.inputs()}")
print(f"Worksheet output names: {test_ws.outputs()}")

# Set some input values
test_ws.set_real_input("real_input_test", 15)
test_ws.set_real_input("real_input_with_units_test", 5, "mm", preserve_worksheet_units=False)
...
# test_ws.save_as(Path.cwd() / "test_output.mcdx")
# test_ws.close()